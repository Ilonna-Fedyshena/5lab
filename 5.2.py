...
def harmonic_noise(x, amplitude, frequency, phase, noise_mean, noise_covariance, show_noise, noise=None):
...
y_clean, current_noise, y_noisy = harmonic_noise(x, amplitude, frequency, phase, noise_mean, noise_covariance, show_noise, current_noise)

source_clean = ColumnDataSource(data=dict(x=x, y=y_clean))
source_noise = ColumnDataSource(data=dict(x=x, y=y_noisy))

p_clean = figure(title="Harmonic without Noise", width=700, height=300)
p_clean.line('x', 'y', source=source_clean, line_width=2, color="green")

p_noise = figure(title="Harmonic with Noise", width=700, height=300)
p_noise.line('x', 'y', source=source_noise, line_width=2, color="blue")
...
def update(attr, old, new):
    ...
    if mean != noise_mean or cov != noise_covariance:
        current_noise = np.random.normal(mean, np.sqrt(cov), len(x))

    y_clean, _, y_noisy = harmonic_noise(x, amp, freq, phase, mean, cov, show_noise, current_noise)

    source_clean.data = dict(x=x, y=y_clean)
    source_noise.data = dict(x=x, y=y_noisy)

for widget in [amp_slider, freq_slider, phase_slider, mean_slider, cov_slider, noise_checkbox]:
...
layout = column(
    ...
    row(noise_checkbox, reset_button)
)
